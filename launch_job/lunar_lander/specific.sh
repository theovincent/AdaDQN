...
do
    launch_job/lunar_lander/cluster_adadqn.sh --experiment_name hp_update_freq_$HP_UPDATE_FREQUENCY\_elitism_reset \
    ...
    launch_job/lunar_lander/cluster_adadqn.sh --experiment_name hp_update_freq_$HP_UPDATE_FREQUENCY\_truncation_reset \
...
do
    launch_job/lunar_lander/cluster_searldqn.sh --experiment_name min_steps_eval_$MIN_STEPS_EVALUATION\_elitism_reset \
        $SHARED_ARGS --n_epochs 50 --n_networks 5 --exploitation_type elitism --min_steps_evaluation $MIN_STEPS_EVALUATION \
        --training_proportion 0.5

    launch_job/lunar_lander/cluster_searldqn.sh --experiment_name min_steps_eval_$MIN_STEPS_EVALUATION\_truncation_reset \
