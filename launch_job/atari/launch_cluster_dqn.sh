...
then
    sbatch -J $EXPERIMENT_NAME\_$ALGO_NAME --array=$FIRST_SEED-$LAST_SEED --cpus-per-task=4 --mem-per-cpu=3G --time=13:00:00 --prefer="rtx3090|a5000" --gres=gpu:1 -p gpu \
    ...
    sbatch -J $EXPERIMENT_NAME\_$ALGO_NAME --array=$FIRST_SEED-$LAST_SEED --cpus-per-task=7 --mem-per-cpu=3G --time=17:00:00 --prefer="rtx3090|a5000" --gres=gpu:1 -p gpu \
