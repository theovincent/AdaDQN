...
class DQN:
    def __init__(
        ...
        self.q = SingleDQN(n_actions, hidden_layers, activations, loss)
        self.params = self.q.q_network.init(key, jnp.zeros(observation_dim, dtype=jnp.float32))
    ...
    def best_action(self, params: FrozenDict, state: jnp.ndarray):  # computes the best action for a single state
        return self.q.best_action(params, state)
