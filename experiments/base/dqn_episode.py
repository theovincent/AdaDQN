...
def train(
    ...
    for idx_epoch in tqdm(range(p["n_epochs"])):
        ...
        avg_return = sum(episode_returns_per_epoch[idx_epoch]) / len(episode_lengths_per_epoch[idx_epoch])
        print(
            f"\nEpoch: {idx_epoch} Avg. return = {avg_return}"
            + f" Num episodes = {len(episode_lengths_per_epoch[idx_epoch])} \n",
            flush=True,
        )

        if idx_epoch < p["n_epochs"] - 1:
            agent.update_hyperparamters(idx_epoch + 1, avg_return)
